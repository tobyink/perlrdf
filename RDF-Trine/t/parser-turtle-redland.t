...
if ($RDF::Trine::Parser::Redland::HAVE_REDLAND_PARSER) {
    plan qw(no_plan);
} elsif ($ENV{RDFTRINE_NO_REDLAND}) {
    plan skip_all => "Redland parser disabled by \$RDFTRINE_NO_REDLAND.";
}else {
    plan skip_all => "Redland parser is not available.";
}


my $parser = RDF::Trine::Parser::Turtle::Redland->new;
isa_ok $parser, 'RDF::Trine::Parser::Turtle::Redland';
isa_ok $parser->redland_parser, 'RDF::Redland::Parser';
...

{
    ...
    my $parser	= RDF::Trine::Parser::Turtle::Redland->new( canonicalize => 1 );
    {
